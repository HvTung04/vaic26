...
export interface ResultHistoryListProps {
  ...
  isLoading?: boolean;
  /** Omit to hide the "view all history" link (e.g. teacher view, which has no such page). */
  viewAllHref?: string;
  /** Where a row links to. Defaults to the student's own submission detail page. */
  getRowHref?: (submissionId: string) => string;
  /** Max rows shown (there's a separate "view all" page for the rest). Pass `history.length` to show everything. */
  limit?: number;
}
          ...
          history?.slice(0, limit).map((item) => {
            ...
            return (
              <Link
                ...
                to={getRowHref(item.submissionId)}
